...
\newcommand{\mba}{méthode de \textit{\ac{BA}} }
\newcommand{\mbae}{méthode de \textit{\ac{BAE}} }
...
\begin{document}
	...
	Son efficacité actuelle repose sur un tryptique fondamental. Le premier axe consiste à modéliser mathématiquement ces processus complexes afin d'en traduire fidèlement les contraintes et les objectifs.
	Le deuxieme réside dans le développement de méthodes de résolution capables de passer à l'échelle industrielle. Enfin, le dernier levier de performance, devenu central aujourd'hui, est le traitement intégré et intelligent des données.
	
	C'est précisément sur ce dernier point que la Recherche Opérationnelle converge vers l'Intelligence Artificielle. Cependant, l'exploitation de ces données issus d'environnements réels confronte systèmatiquement les modèles à un défi majeur: l'omniprésence de l'incertitude.
	En effet, l'instabilité de la demande, des prix et du climat complexifie la planification au quotidien. La production, la logistique et les approvisionnements subissent des risques accrus amplifiés par l'incerdépendance des chaînes mondiales.
	Les modèles classiques supposent souvent un monde \enquote{parfait}. Optimiser ce scénario crée une stratégie fragile qui ne fonctionnera plus au premier aléa. L'enjeu n'est donc plus seulement d'optimiser un plan idéal, mais également d'anticiper les écarts malgré ces intertitudes, c'est la naissance de l'\ac{OR}.
	
	Ce nouveau concept permet de poser un cadre pour décider dans l'incertain. Nous pourrions le définir de la façon suivante: l'Optimisation Robuste cherche la meilleure décision qui reste faisable et performante quelques soient les scénarios à venir.
	...
	Les applications couvrent notamment la supply chain, la maintenance et la production où les aléas locaux peuvent déclencher des effets systèmatiques sur la performances globales.
	
	Pour résoudre ces problèmes d'Optimisation Robuste, il existe de nombreuses approches. L'une d'entre elles, appelée la méthode de \ac{BA} (ou parfois méthode des plans coupants) a été introduite dans \cite{Mutapcic01062009}. C'est cette approche que nous avons utilisé et que nous détaillerons par la suite.
	
	Comment les méthodes de Benders peuvent optimiser la planficiation robuste sous incertitude?
	Nous présenterons tout d'abord le contexte de la recherche, ferons un point sur les avancées scientifiques dans ce domaine, puis nous présenterons les objectifs, les solutions ainsi que les résultats obtenus. Les apports techniques et professionnels de cette expérience feront l'object de la dernière partie.
	...
	L'impulsion de cette dynamique est notamment portée par la chair HEROIC\footnote{Hybridizing Learning, seaRch and combinatorial Optimization for Industrial deCision-making.} d' ANITI (Artificial and Natural Intelligence Toulouse Institute), un cluster labellisé par la Communauté d'universités et d'établissements de Toulouse. Son ambition est de concevoir des systèmes d'Intelligence Artificielle dont les prises de décision sont explicables, équitables, fiables et robustes, une condition indispensable pour permettre leur déploiement dans des secteurs critiques à haut risque.
	...
	IMPORTANT : parler de l'utilisation à terme du modèle par AIRBUS. + implication dans la suite de la thése de tom portolleau
	...
	L'optimisation robuste absolue (où approche \mmx) de problèmes de planification est complexe. Pour contourner cette difficulté, la méthode de Benders Adverse s'appuie sur une décomposition itérative : elle isole un problème Maître, qui planifie la production, et un problème adverse, qui évalue la robustesse de cette planification en cherchant le pire scénario possible.
	
	Dans la littérature, \cite{PATZOLD202020} présentent une variante de cette méthode où les pires scénarios sont calculés de manière approximative tout en garantissant la convergence de l'algorithme. Plus récemment, les travaux de \cite{portoleau:hal-03595446}, sur lesquels s'appuie l'essentiel de notre étude, proposent la méthode de Benders Adverse Etendue, que nous noterons $BAE$ ou $KC$ pour \textit{Knowledge Compilation}. Contrairement à l'approche classique, l'adversaire ne renvoie plus un unique pire cas, mais un ensemble de scénarios défavorables afin d'accélérer la convergence globale. Dans la continuité de cette recherche, nous présenterons cette méthode pour un problème de dimensionnement de lots avec contraintes de capacité (\textit{Capacitated Lot Sizing- Problem - CLSP}), soumis à un budget d'incertitude portant sur la demande cumulée.
	
	\subsection{Définition et Modélisation du Problème}
	
	Afin de modéliser ce problème, nous considérons un horizon de planification divisé en $T$ périodes. A chaque période $t\leq T$, la demande est notée $d_t$ et la quantité produite $x_t$. Un plan de production est constitué par l'ensemble de ces variables décisionnelles sur l'horizon. La production à chaque pas de temps est bornée par une capacité maximale, de sorte que $x_t\leq \mathbf{X}$. La gestion des flux de production génère différents coûts linéaires indépendants de la période. On a $c^P$ le coût unitaire de lancement de production, $c^I$ le coût unitaire de stockage appliqué au surplus de production (noté $I_t$), $c^B$ le coût unitaire de pénalité de retard appliqué à la demande insatisfaite (notée $B_t$), et enfin $b^P$ le prix de vente unitaire appliqué à la quantité de demande effectivement satisfaite (notée $s_t$). Contrairement aux approches classiques où l'incertitude porte sur la demande de chaque période de façon indépendante, notre modèle intègre une incertitude sur la demande \textbf{cumulée}. Soient $X_t = \sum_{i=1}^{t}x_i$ la production cumulée et $D_t = \sum_{i=1}^{t}d_i$ la demande cumulée à l'instant $t$. L'incertitude sur la demande cumulée est modélisée par un intervalle symétrique  $[\hat{\mathcal{D}}_t - \Delta_t, \hat{\mathcal{D}}_t + \Delta_t]$, où $\hat{\mathcal{D}}_t \geq \Delta_t \geq 0$ représente la valeur nominale et $\Delta_t$ la déviation maximale.
	...
	Une fois un plan de production initial $S$ obtenu, une phase adverse (le sous-problème \adv) est exécuté. Son rôle est d'identifier, pour ce plan $S$ figé, le scénario $\mathcal{D}'$ maximisant les coûts. Ce pire scénario est alors ajouté à l'ensemble restreint des scénarios considérés, et le problème Maître est résolu à nouveau. Ce processus itératif, illustré par la figure \ref{fig:sch_ba}, s'arrête lorsque l'adversaire n'est plus en mesure de trouver un scénario dégradant la valeur objective du problème Maître, garantissant ainsi que la dernière solution calculée est robuste.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\linewidth]{images/schema_ba.png} 
		\caption{Schéma de la méthode de Benders Adverse. \cite{portoleau:hal-03595446}}
		\label{fig:sch_ba}
	\end{figure}
	
	
	
	Nous allons commencer par définir la méthode de Benders adverse de façon théorique et appliquée à notre problématique. L'idée de cette méthode est de résoudre le problème \mmx (ou Problème Maître), d'abord sur un seul scénario (la demande nominale $\hat{\mathcal{D}}$), puis en augmentant le nombre de scénarios considérés. Pour calculer le prochain scénario $\hat{\mathcal{D}'}$ à agréger au problème Maître, il faut effectuer une phase adverse, c'est-à-dire que l'on cherche, sachant la solution particulière $S$ du problème calculé à l'étape précédente, à calculer le pire scénario pour cette solution. Ce problème est appelé le problème adverse et sera noté \adv. Le scénario retenu est alors ajouté à l'ensemble des scénarios $\mathcal{U}$. Les itérations s'arrêtent lorsque le critère d'arrêt est atteint, c'est-à-dire lorsque le problème Maître et le problème Adverse ont convergé vers la même solution donc quant il n'est plus possible de dégrader davantage la valeur objective du problème Maître. De ce fait, la dernière solution trouvée en résolvant le problème \mmx correspond à la solution robuste. L'algorithme décrit ci-dessus peut-être schématisé de la façon suivante :
	...
	L'application du problème \mmx sur de la planification robuste peut-être définie de la façon suivante : \enquote{Trouver le plan le moins couteux face au pire des scénarios.}. 
	
	Nous pouvons le modéliser comme suit :
	
	\begin{align}
		\text{minimize}\,\,&\mathbf{c}^\mathbf{x}+\mathbf{d}^T\mathbf{y}\\
		\text{s.t.}    \,\,&A\mathbf{x}+B\mathbf{y}\geq \mathbf{b}\\
		&\mathbf{y}\in \mathbf{Y}\\
		&\mathbf{x}\geq0
	\end{align}
	
	Où $A, B$ représentent les contraintes partagées par les deux étapes de variables et $Y$ représente l'ensemble des possibles pour $\mathbf{y}$. Notez que $\forall \overline{\mathbf{y}}\in \mathbf{Y}$ fixe, le problème résiduel est :
	
	\begin{align}
		\text{minimize}\,\,&\mathbf{c}^T\mathbf{x}+\mathbf{d}^T\mathbf{y}\\
		\text{s.t.}    \,\,&A\mathbf{x}+B\mathbf{y}\geq \mathbf{b}\\
		&\mathbf{y}\in \mathbf{Y}\\
		&\mathbf{x}\geq0
	\end{align}
	...
	\begin{align}
		\text{maximize}\,\,&(\mathbf{b}-B\overline{\mathbf{y}})^T\mathbf{u}+\mathbf{d}^T\overline{\mathbf{y}}\\
		\text{s.t.}    \,\,&A^T\mathbf{u}\leq\mathbf{c}\\
		&\mathbf{u}\geq0
	\end{align}
	
	Nous pouvons finalement représenter le problème initial comme un problème \mmx équivalent :
	
	\begin{equation}
		\min_{\mathbf{y}\in Y}[d^T\mathbf{y}+\max_{\mathbf{u}\geq0}\{(b-B\mathbf{y})^T\mathbf{u}\,|\, A^T\mathbf{u}\leq c\}]
	\end{equation}
	
	Par application de la théorie à notre cas d'étude nous pouvons modéliser le programme linéaire en variable mixtes (MILP) grâce aux variables suivantes. Sachant un ensemble de scénarios $U\subseteq\mathcal{U}$ et un horizon $T$, on a pour $1\leq t\leq T$ et $1\leq l\leq |U|$, $X_t=\sum_{i=1}^{t}x_i$ la production cumulée jusqu'à la période $t$, $D_t=\sum_{i=1}^{t}d_i$ la demande cumulée jusqu'au temps $t$, $y_t\in\{0, 1\}$ l'indicatrice de production à la période $t$, $I_t, B_t, s_t \geq 0$ respectivement le niveau des stocks, le niveau des commandes en retards et les ventes du produit à la fin de la période $t\in[T]$ et enfin $M$ la variable du \textit{BigM}.
	...
	\vspace{-0.5em}
	{\centering
		\small Lot-Sizing Problem.\cite{portoleau:tel-04631599}\par
		\label{formulation_probleme_maitre}
	}
	
	Nous voyons bien que l'objectif est de minimiser les bénéfices nets du Maître. En effet, pour un $l$ donné, la quantité à gauche dans la contrainte II.14 vaut la valeur de la fonction objective de la solution $X$ dans le $l$-ième scénario de $U$. Nous résolvons donc bien le problème $\min_{X\in\mathcal{X}}\max_{u\in U}$ (à mieux expliquer pédagogiquement sans rentrer dans les détails car je ne vois pas le lien). La contraire $(13)$ s'assure de la conservation des flux, tandis que la suivante définit les ventes cumulées effectives à la fin de la période $t$.
	
	\subsubsection{Le sous-problème \adv}
	
	
	Comme dit précédemment, le problème \adv cherche à calculer le chemin le plus long dans un graphe partitionné construit à partir d'un plan de production $X$. Cette notion de graphe de budget a été définie par \cite{guillaume2020robustproductionplanningbudgeted} de la façon suivante. L'idée de cet algorithme vient des mêmes auteurs qui ont prouvé que ce problème pouvait être résolu en temps pseudo-polynomiale du fait que nous pouvions réduire ce problème $NP$-Difficile au sens faible à une variante du problème du sac-à-dos (\textit{knapsack}).
	
	
	\begin{definition}{Graphe de budget}{graphe_budget}
		On note ce graphe $\mathcal{G}=(V, A)$. L'ensemble des noeuds $V$ partitionné en $T+2$ niveaux et, mis à part le premier et le dernier niveau qui contiennent un unique noeud chacun, chaque niveau contient $\Gamma +1$ noeuds. On note respectivement $v_0$ et $v_{T+1}$ les noeuds du premier et du dernier niveau, et $v_{t, i}$ pour $1\leq t\leq T$ et $0\leq i \leq \Gamma$ tous les autres noeuds du graphe. L'ensemble des arcs $A$ est construit de sorte que l'arc de $v_{t-1, i}$ vers $v_{t, j}$ existe et si et seulement si $i\leq j$ et $j-i\leq\Delta_t$ pour chaque $t$ compris entre $1$ et $T$, et un arc de $v_0$ vers $v_{T+1}$ représente un scénario de $\mathcal{U}$, et que si un noeud $v_{t, i}$ est sur ce chemin, alors $i$ unités de budget ont été consommées entre la première période et la période $t$. Ensuite, on définit les coûts des arcs de sorte qu'il représentent chacun la contribution à la fonction objective de la décision de faire varier la demande à une période donnée pour une solution $X$ donnée. Plus formellement, on définit les coûts des arcs par :
	...
	Nous pouvons schématiser plus simplement cette définition par la figure \ref{fig:sch_graphe_budget}. Pour la suite du rapport, le terme de \enquote{chemin} signifiera un chemin dans ce graphe entre la source $v_0$ et le puit $v_{T+1}$.
	...
	Voici l'algorithme implémenté par \cite{portoleau:tel-04631599} pour résoudre le problème de Benders Adverse présenté ci-dessus dans la figure \ref{fig:sch_ba}.
	
	\begin{algorithm}[H]
		\caption{Adversarial Benders Method}
		\label{alg:benders_algo}
		\begin{algorithmic}[1]
			\State $U \gets \{\hat{\mathcal{D}}\}$
			\State $(X,v) \gets \Call{SolveMaster}{U}$
			\State $(\mathcal{D},v') \gets \Call{SolveAdversarial}{X}$
			
			\While{$v \neq v'$}
			\State $v \gets v'$
			\State $U \gets U \cup \{\mathcal{D}\}$
			\State $(X,v) \gets \Call{SolveMaster}{U}$
			\State $(\mathcal{D},v') \gets \Call{SolveAdversarial}{X}$
			\EndWhile
	...
	Cette section détaille la méthode de Benders Adverse Étendue (BAE), formellement introduite par \cite{portoleau:tel-04631599}. Cette approche constitue le fondement de mes recherches. Il s'agit d'une extension de la méthode de Benders Adverse classique présentée précédemment, conçue pour accélérer la convergence de l'algorithme.
	
	Plutôt que d'isoler l'unique pire scénario à chaque itération du sous-problème adverse, la BAE génère un ensemble de scénarios défavorables. L'enjeu de cette méthode réside dans la résolution du problème Maître : sa formulation initiale, basée sur une liste explicite de scénarios  $\mathcal{U}$, souffre d'une explosion combinatoire et passe très difficilement à l'échelle lorsque ce nombre augmente. Pour faire face cette limite, le problème Maître est redéfini en s'appuyant sur une structure plus compacte : le graphe partiel de budget.
	
	\subsubsection{Le problème \mmx}
	
	Pour ce faire, les auteurs de \cite{portoleau:tel-04631599} ont précisemment défini cette notion. Lorsque le problème Maître du Benders Adverse classique se basait sur l'approximation de l'ensemble des scénarios $\mathcal{U}$ par une liste de scénarios, cette fois-ci, elle se fait par l'extraction d'un sous-graphe issu du graphe de budget global (voir \ref{fig:sch_graphe_budget}).
	
	\begin{definition}{Graphe partiel de budget}{graphe_partiel_budget}
		Soit $\mathcal{G}=(V, A)$ un graphe de budget défini suivant la définition \ref{graphe_budget}. On dit que $\mathcal{G}'=(V, A')$ est un graphe partiel de budget de $\mathcal{G}$ si $A'\subset A$ et pour tout arc $a\in A'$, il existe un chemin de $v_0$ vers $v_{T+1}$ passant par l'arc $a$. Les poids des arcs de ce graphe sont calculés de la même manière que pour le graphe défini précédemment. On a alors $\mathcal{U}_{\mathcal{G}'}\subset \mathcal{U}_{\mathcal{G}}$.
	\end{definition}
	
	Grâce à cette structure, le problème Maître peut être reformulé comme un problème de calcul de plus long chemin dans le graphe partiel $\mathcal{G}'$. Cette formulation s'appuie sur des variables de potentiel, notées $\pi_{t,i}$, qui représentent le coût du plus long chemin depuis le nœud source $v_0$ jusqu'au nœud $v_{t,i}$. Le programme linéaire en variables mixtes modélisant ce nouveau problème Maître s'écrit alors de la manière suivante :
	...
	\vspace{-0.5em}
	{   \centering
		\small Reformulation du problème Maître.\cite{portoleau:tel-04631599}\par
		\label{reformulation_pbr_maitre}
	}
	
	\subsubsection{Le sous-problème \adv}
	
	Une fois le problème Maître résolu pour une itération donnée, il fournit un plan de production $X$. L'objectif du sous-problème adverse est alors d'évaluer la robustesse de cette solution en cherchant non pas un unique pire scénario, mais un ensemble de scénarios défavorables qui viendront aggrandir le problème Maître à l'itération suivante. Cet ensemble prend la forme d'un nouveau graphe partiel de budget, noté $\mathcal{G}''$.
	
	Pour construire ce graphe, la méthode s'appuie sur un algorithme de programmation dynamique. Dans un premier temps, un calcul de plus long chemin est réalisé sur le graphe de budget complet $\mathcal{G}$, afin de déterminer le coût maximal pour atteindre chaque noeud depuis l'origine $v_0$. Cette étape s'effectue rapidement du fait que le graphe soit orienté et sans cycle. Ensuite, une phase de remontée (\textit{backtracking}) est réalisée depuis le noeud final $v_{T+1}$. A chaque étape de cette remontée, au lieu de ne conserver que l'arc menant au pire chemin absolu, l'algorithme sélectionne un sous-ensemble d'arcs appartenant aux chemins critiques. C'est cette sélection d'arcs qui définit la structure et la taille du graphe partiel $\mathcal{G}''$. Pour être plus précis, une heuristique a été mise en place sur la définition des pires chemins. Lors de la première phase, le coût du plus long chemin, noté $C$, va servir de référence pour définir la notion de chemin critique. Considérons $\tau\in[0, 1]$, un chemin critique sera donc un scénario dont sa longueur est supérieure ou égale à $\tau C$ dans le grape de budget. Si $\tau=1$ alors seuls les chemins ayant le même coût que celui du pire chemin sont considérés comme critiques et donc ajoutés au sous-graphe. A l'inverse, si $\tau=0$, tous les chemins sont considérés comme étant des chemins critiques. Nous nous attendons à ce que la diminution de $\tau$ entraine une augmentation du temps de résolution mais une potentielle baisse du nombre d'itération.
	...
	Toutefois, cette opération de fusion soulève une limite, sur laquelle j'ai travaillé, concernant l'aspect combinatoire de cette méthode \cite{portoleau:tel-04631599} : la fusion de deux graphes partiels peut générer de nouveaux chemins qui n'existaient dans aucun des deux graphes initiaux. Ainsi, l'ensemble des scénarios du graphe fusionné est potentiellement plus grand que la somme des scénarios des graphes de départ. Il devient donc nécessaire de travailler sur l'affinage des coupes combinatoires afin de ne pas perdre l'avantage porté par cette extension.
	
	\subsubsection{Stratégies de sélection des pires scénarios}
	
	Comme évoqué précédemment, la taille et la densité du graphe partiel de budget $\mathcal{G}''$ dépendent entièrement de la politique de sélection des arcs lors de la phase de backtracking. Dans la littérature, deux heuristiques principales ont été proposées pour effectuer ce tri parmis les chemins critiques \cite{portoleau:tel-04631599} :
	...
	\begin{itemize}
		...
		\item $SelectFew$ : Pour faire face à ce problème, cette méthode propose de restreindre la sélection. Lors du parcours, un premier arc critique est systématiquement choisi, puis un autre arc critique potentiel n'est ajouté qu'avec une certaine probabilité, suivant une loi de Bernouilli de paramètre $p$ (les travaux de référence utilisent généralement $p=\frac{1}{T}$, où $T$ est le nombre de période dans l'horizon). Cette restriction probabiliste permet de garder un graphe partiel de taille réduite, car il existe un grand nombre de pires scénarios équivalents.
	\end{itemize}
	
	Ces deux méthodes illustrent le compromis soutenu par les auteurs $BAE$. Il s'agit de fournir suffisamment de scénarios critiques au Maître pour le guider efficacement, tout en contrôlant la taille du graphe pour garantir des temps de résolution acceptables. Cette nécessité de filtrage met en évidence l'intérêt de développer des heuristiques de sélection plus structurelles. Au lieu de s'en remettre à une sélection probabiliste ou exhaustive, il devient pertinent de chercher à extraire des chemins non seulement critiques, mais le plus diversifiés possibles, afin de maximiser la quantité d'information transime tout en minimisant la redondance des scénarios. C'est ce point que nous aborderons dans la prochaine section. Dans la suite du rapport, $BAE$ signifiera l'approche Benders Adverse Etendue avec l'heuristique $SelectAll$, et $BAE_{Few}$ permettra de spécifier l'utilisation de l'heuristique $SelectFew$.
	
	\subsubsection{L’algorithme de Benders Adverse Etendue pour le lot-sizing sous incertitude }
	
	En combinant les éléments précédents, nous avons le schéma itératif de la méthode de Benders Adverse Etendue. De la même manière que nous initialisons l'algorithme de Benders Adverse (\ref{alg:benders_algo}) par le scénario nominal, ici, nous initialisons la méthode de Benders Adverse Etendue par le graphe de budget ne contenant que le scénario nominal, noté $\hat{\mathcal{G}}$.
	...
	Dans cette section, nous présenterons les objectifs de cette recherche, nos propositions ainsi que les résultats obtenus. Elle s'est déroulée en 3 temps.
	
	La première partie de ce stage a évidemment été allouée à la compréhension des nouvelles notions et concepts. Il a été nécessaire de réaliser l'état de l'art concernant la notion de \textit{lot-sizing} robuste. Afin de mieux conceptualiser l'algorithme de la méthode de Benders Adverse, nous avons commencé par l'appliquer manuellement sur des instances jouets. Le but était de se familiariser avec la logique des différentes passes Maître-Adversaire.
	
	
	
	
	
	Comme vu précédemment, la méthode alternative proposée consistant à ramener un graphe des pires scénarios promeut que la réduction du nombre d'itération permet de réduire le temps de calcul. Nous avons exploités cette heuristique et avons obtenus les résultats obtenus dans le tableau suivant. L'étude s'est faite pour $\gamma$ prenant ses valeurs dans son ensemble de définition suivant $\mathcal{D}_{\gamma}=\{0, 10 ,20, \cdots, 90, 100\}$, $\tau$ variant dans $\mathcal{D}_{\tau}=\{0\%, 10\%, 20\%, \cdots, 90\%, 100\%\}$. Nous étudions donc les couples de la forme $(\gamma_i, \tau_j), \forall i\in\mathcal{D}_{\gamma}, \forall j\in\mathcal{D}_{\tau}$. L'heuristique de sélection des scénarios $SelectFew$ suit une loi de Bernouilli de paramètre $\mathcal{B}_{SFew}(p=0.5)$. Les instances ont été gérénées aléatoirement sur $52$ périodes représentant une année de planificiation, avec $20$ références à planifier et une possiblité de demande pour chaque référence à chaque temps suivant également une loi de Bernouilli de paramètre $\mathcal{B}_{Demand}(p=0.5)$. Pour cette étude, les deux métriques utilisées sont le nombre d'itération ainsi que le temps de résolution de l'instance pour le couple de paramètre donné.
	...
	\begin{table}[H]
		...
		\end{tabular}
		\caption{Nombre d'itérations et temps d'exécution (en secondes) moyen pour chacune des trois méthodes. (Table extraite de \cite{portoleau:hal-03595446})}
	\end{table}
	
	Nous voyons au travers de cette étude que $BAE$ et $BAE_{Few}$ ne sont pas meilleurs que l'approche classique sur tous les couples de paramètres.
	...
	L'idée sous-jacente est qu'en ramenant, parmis les chemins critiques, seulement ceux qui sont les plus éloignés, nous pourrons attaquer le Maître à des périodes complétements différentes et ainsi réduire le temps de calcul. Nous gardons en tête que cette méthode peut faire augmenter le nombre d'itération. En effet, bien que l'approche de Benders Adverse Etendue soit innovante, elle n'infère pas sur la diversité des scénarios remontés tant qu'ils sont supérieurs à $\tau C$.
	HOG a été construite autour de trois éléments : une métrique de distance, une exploration gloutonne ainsi qu'une nouvelle approche \mmx. Plusieurs métriques de distances ont été considérées, comme la distance de Jaccard définit comme le complémentaire de son indice de la façon suivante :
	...
	\noindent où \(d_H\) est la distance de Hamming\footnote{La distance de Hamming est défini dans l'état de l'art (voir \cite{hamming_dist_scheduling_prbl}) comme une métrique de type \enquote{arc overlap}, c'est-à-dire que la distance est défini comme étant la différence d'arcs empruntés entre les deux chemins. Formellement, il s'agit d'une distance $L1$ restreinte aux vecteurs binaires $x\in\{0, 1\}^{|E|}$ :
	...
	\begin{algorithm}[H]
		\caption{Calculation of the Bray-Curtis distance}
		\label{alg:bray_curtis}
		\begin{algorithmic}[1]
			\Require Path $A$, Path $B$
			\Ensure Dissimilarity $d_{BC} \in [0, 1]$
			...
			\State $sum\_total \gets sum\_total + (\Delta_A + \Delta_B)$
			\EndFor
			...
			\State \Return $\frac{sum\_diff}{sum\_total}$
		\end{algorithmic}
	\end{algorithm}
	...
	Les différents programmes linéaires ont été résolus avec \textit{CPLEX 20.1.0} et tous les calculs ont été effectués avec un processeur XXXX sous Linux Ubuntu 24.04.4. Pour ces expérimentations, nous avons considéré deux paramètres. Tout d'abord le budget d'incertitude $\Gamma$ qui prend ses valeurs dans l'ensemble $\{1, 21, 41, 61, 81,101\}$, ainsi que la méthode de sélection proposée par \cite{portoleau:tel-04631599}. Nous avons au total trois méthodes :
	
	\begin{itemize}
		\item[] $BAE$ : la méthode de Benders adverse standard ;
		\item[] $BAE_{all}$ : la méthode de Benders adverse étendue, en utilisant la méthhode de sélection $SelectAll$ ;
		\item[] $BAE_{few}$ : la méthode de Benders adverse étendue, en utilisant la méthode de sélection $SelectFew$.
	\end{itemize}
	
	Les deux critères qui sont observés sont le nombre d'itérations de l'algorithme ainsi que le temps de calcul pour chacune des méthodes et chacun des $\Gamma$. Les résultats sont affichés dans la table XX avec à gauche dans chaque cellule le nombre d'itérations moyen, et à droite le temps de calcul moyen de ces méthodes sur l'ensemble des instances. Pour chaque $\Gamma$, les valurs des cellules avec le plus petit nombre d'itérations moyens ou le plus petit temps de calcul moyen ont été mises en gras.
